...
import { CATEGORY_OPTIONS, type TodoCategory } from './useTodos';

export type TodoStatus = 'Overdue' | 'Today' | 'Upcoming' | 'Completed';
...
interface TodosContextType {
    todos: TodoItem[];
    loading: boolean;
    addTodo: (todoData: Omit<TodoItem, 'id' | 'is_completed'>) => Promise<void>;
    toggleTodo: (id: string) => Promise<void>;
    deleteTodo: (id: string) => Promise<void>;
    updateTodo: (id: string, updates: Partial<TodoItem>) => Promise<void>;
    getTodaysTodos: () => TodoItem[];
    getFilteredTodos: (filter: string) => TodoItem[];
...
export const TodosProvider = ({ userId, children }: { userId: string; children: ReactNode }) => {
    ...
    const addTodo = async (todoData: Omit<TodoItem, 'id' | 'is_completed'>) => {
        ...
        const newId = uuidv4();
        // Format due date as local YYYY-MM-DD to avoid timezone shift
        const pad = (n: number) => n.toString().padStart(2, '0');
        const dueDate = todoData.dueDate
            ? `${todoData.dueDate.getFullYear()}-${pad(todoData.dueDate.getMonth() + 1)}-${pad(todoData.dueDate.getDate())}`
            : null;
